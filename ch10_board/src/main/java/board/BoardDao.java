...
public class BoardDao {
	...
	public ArrayList<Board> getBoardList(String keyField, String keyWord, int start, int end){
		...
		try {
			...
			}else
			{
				sql = "select * "
		                + "  from (select ROWNUM AS RNUM, BT1.* "
		                + "         from (select * from board where " + keyField + " like ? order by ref desc, pos) BT1"
		                + "        )"
		                + "  where RNUM between ? and ?";
	...
	public boolean checkCanDeleteBoard(Board board) {
		boolean flag = false;
		try {
			con = pool.getConnection();
			
			sql = "select count(*) from board where ref = ? and depth > ?";
			pstmt = con.prepareStatement(sql);
